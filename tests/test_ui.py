...
# ------------------------------------------------- state survival regressions
@needs_data
def test_navigation_keeps_settings_and_selection():
    """Streamlit drops the state of widgets a run did not draw.

    The dock defers its reruns until every widget exists, and the selection is
    mirrored outside widget state, so switching section cannot silently reset
    the benchmark, currency, period or the comparison list.
    """
    at = AppTest.from_file(APP, default_timeout=600)
    at.session_state["lang"] = "EN"
    at.run()
    selection = list(at.session_state["selection"])
    settings = {k: at.session_state[k] for k in ("benchmark", "currency", "period")}

    for view in ["nav_screener", "nav_data", "nav_compare", "nav_profile", "nav_lab"]:
        at.button(key=f"nav_{view}").click().run()
        assert not at.exception, [str(e.value) for e in at.exception]
        assert at.session_state["view"] == view
        assert list(at.session_state["selection"]) == selection, view
        for key, value in settings.items():
            assert at.session_state[key] == value, f"{key} lost on {view}"


@needs_data
def test_language_switch_keeps_settings():
    at = AppTest.from_file(APP, default_timeout=600)
    at.run()
    benchmark = at.session_state["benchmark"]
    at.button(key="lang_DE").click().run()
    assert at.session_state["lang"] == "DE"
    assert at.session_state["benchmark"] == benchmark
    assert not at.exception, [str(e.value) for e in at.exception]


@needs_data
def test_scope_filter_narrows_every_view():
    at = AppTest.from_file(APP, default_timeout=600)
    at.session_state["lang"] = "EN"
    at.session_state["f_region"] = ["Europe"]
    at.session_state["view"] = "nav_screener"
    at.run()
    assert not at.exception, [str(e.value) for e in at.exception]

    ds = rp.load_dataset()
    allowed = set(ds.profile[ds.profile.region == "Europe"].ticker)
    universe = S.filtered_universe.__wrapped__ if hasattr(
        S.filtered_universe, "__wrapped__") else None
    # the screener count tile must not exceed the filtered universe
    shown = int(at.metric[0].value)
    assert shown <= len(allowed)


def test_set_selection_clears_the_picker_widget():
    import streamlit as st
    st.session_state.clear()
    st.session_state[S.PICKER_KEY] = ["OLD"]
    S.set_selection(["A", "B", "A"])
    assert st.session_state["selection"] == ["A", "B"]
    assert S.PICKER_KEY not in st.session_state


@needs_data
def test_region_filter_prunes_the_comparison():
    """Choosing a region means the report shows that region — including the
    comparison list, not just the pickers."""
    at = AppTest.from_file(APP, default_timeout=600)
    at.session_state["lang"] = "EN"
    at.run()
    assert len(at.session_state["selection"]) > 1
